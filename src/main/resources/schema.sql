...
(
    ...
    -- ★ FIX: align with entity/repository (source, external_id)
    CONSTRAINT uq_vuln_source_external UNIQUE (source, external_id)
    );

-- NOTE: UNIQUE で (source, external_id) を持つため、external_id単体の索引は原則不要
-- CREATE INDEX IF NOT EXISTS idx_vuln_external_id ON vulnerabilities(external_id);

CREATE INDEX IF NOT EXISTS idx_vuln_severity ON vulnerabilities(severity);

-- ★ TUNE: paging/sort (order by last_modified_at desc, id desc)
...
-- ★ MUST: idempotency key for existsBy(...) and dedupe
...
-- search indexes (keep minimal + matching needs)
-- NOTE: idx_vac_cpe は lower(cpe_name) like '%x%' には効きにくいが、
-- findByCpeName(完全一致) を使うなら残す価値あり。ここでは現状維持で残す。
CREATE INDEX IF NOT EXISTS idx_vac_cpe ON vulnerability_affected_cpes(cpe_name);

-- ★ TUNE: replace idx_vac_vuln (single) with (vulnerability_id, cpe_name) for
-- findByVulnerabilityIdOrderByCpeNameAsc + existence style queries
CREATE INDEX IF NOT EXISTS idx_vac_vuln_cpe ON vulnerability_affected_cpes(vulnerability_id, cpe_name);
...
-- ★ TUNE: vulnerability exists(openOnly) path
